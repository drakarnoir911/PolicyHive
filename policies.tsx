          ...
          {(['All', 'Active', 'Lapsed', 'Due'] as const).map((item) => {
            ...
            return (
              <Pressable key={item} onPress={() => setFilter(item)} style={[{ borderRadius: 999, borderWidth: 1, paddingHorizontal: 14, paddingVertical: 8 }, { backgroundColor: active ? t.tealDim : t.bgCard3, borderColor: active ? t.teal + '40' : t.borderSub }]}>
                <Text style={[{ fontFamily: PolicyHiveTypography.bodyMedium, fontSize: 12, letterSpacing: 0.2 }, { color: active ? t.teal : t.textMid }]}>{item}</Text>
              </Pressable>
            );
          })}
        </View>
              ...
              <Pressable onPress={() => router.push({ pathname: '/policy/[id]', params: { id: policy.id } })}>
                <BlurView intensity={t.blurIntensity} tint={t.blurTint} style={[g.card, { borderColor: t.border, borderRadius: 22, backgroundColor: t.bgCard2 }]}>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                    <Text style={[{ fontFamily: PolicyHiveTypography.bodyBold, fontSize: 17, letterSpacing: -0.2, flex: 1 }, { color: t.text }]} numberOfLines={1}>{policy.policyName}</Text>
                    <View style={[{ borderRadius: 999, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 4 }, { backgroundColor: activeColor.bg, borderColor: activeColor.border }]}>
                      <Text style={[{ fontFamily: PolicyHiveTypography.bodyMedium, fontSize: 10, letterSpacing: 0.5, textTransform: 'uppercase' }, { color: activeColor.text }]}>{status.label}</Text>
                    </View>
                  </View>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 6 }}>
                    ...
                    <Text style={[{ fontFamily: PolicyHiveTypography.bodyMedium, fontSize: 13 }, { color: t.textMid }]}>{policy.clientName} • {policy.insurer}</Text>
                  </View>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <View style={{ flex: 1, alignItems: 'flex-start' }}>
                      <Text style={[{ fontFamily: PolicyHiveTypography.bodyBold, fontSize: 9, letterSpacing: 1.2, marginBottom: 4 }, { color: t.textDim }]}>ANNUAL PREMIUM</Text>
                      <Text style={[{ fontFamily: PolicyHiveTypography.bodyBold, fontSize: 16, letterSpacing: -0.3 }, { color: t.text }]}>{formatCurrency(policy.annualPremium)}</Text>
                    </View>
                    <View style={{ flex: 1, alignItems: 'flex-end' }}>
                      <Text style={[{ fontFamily: PolicyHiveTypography.bodyBold, fontSize: 9, letterSpacing: 1.2, marginBottom: 4 }, { color: t.textDim }]}>COMMISSION ({policy.commissionRate}%)</Text>
                      <Text style={[{ fontFamily: PolicyHiveTypography.bodyBold, fontSize: 16, letterSpacing: -0.3 }, { color: t.gold }]}>{formatCurrency(getPolicyCommission(policy))}</Text>
                    </View>
                  </View>
        ...
        {filtered.length === 0 && (
          <FadeIn delay={200}>
            <View style={{ alignItems: 'center', paddingVertical: 40, gap: 12 }}>
              ...
              <Text style={[g.bodyMedium, { color: t.textMid, fontSize: 15 }]}>No policies found.</Text>
            </View>
          </FadeIn>
        )}
      </View>
